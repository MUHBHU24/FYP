...
urlpatterns = [
    ...
    path('survey-details/<slug:survey_slug>', api.get_survey_details, name='get_survey_details'), # Get survey details for a specific survey
    path('refresh/', TokenRefreshView.as_view(), name='refreshToken'), # Refresh token
    path('survey/<slug:survey_slug>/', views.download_survey_results, name='download_survey_results'), # download survey results as csv
